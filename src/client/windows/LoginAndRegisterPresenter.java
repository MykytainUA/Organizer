...
public class LoginAndRegisterPresenter {
	...
	private void initialize() {
		...
		LogRegView.getLoginRegisterNewAccountButton().addActionListener(new LoginRegisterNewAccountButtonListener());
		LogRegView.getLoginSignInButton().addActionListener(new LoginSignInButtonListener());
		LogRegView.getRegisterGoBackToSignInButton().addActionListener(new RegisterGoBackToSignInButtonListener());
		LogRegView.getRegisterSignUpButton().addActionListener(new RegisterSignUpButtonListener());
			...
			if(dataMessage.isDataUpdated()) { // If data was updated 
				if(dataMessage instanceof SignInMessage) { // Deal with sign in response
					...
					if(castedData.getResponseState() == ResponseState.SUCCESS) {
						new MainContentWindowView(castedData.getID());
	...
	private class LoginRegisterNewAccountButtonListener implements ActionListener{
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			CardLayout cardLayout = (CardLayout) LogRegView.getFrame().getContentPane().getLayout();
			cardLayout.show(LogRegView.getFrame().getContentPane(), "registerWindowPanel");
		}
	...
	private class RegisterGoBackToSignInButtonListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			CardLayout cardLayout = (CardLayout) LogRegView.getFrame().getContentPane().getLayout();
			cardLayout.show(LogRegView.getFrame().getContentPane(), "loginWindowPanel");
		}
	...
	private class LoginSignInButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(LogRegView.getLoginLoginField().getText().length() < 3) {
				JOptionPane.showMessageDialog(getLogRegView().getFrame(), "Login must be at least 3 digits", "Login too small", JOptionPane.WARNING_MESSAGE);
			} else if (LogRegView.getLoginPasswordField().getPassword().length < 3){
				JOptionPane.showMessageDialog(getLogRegView().getFrame(), "Password must be at least 3 digits", "Password too small", JOptionPane.WARNING_MESSAGE);
			} else {
				setDataMessage(new SignInMessage(LogRegView.getLoginLoginField().getText(), 
						String.valueOf(LogRegView.getLoginPasswordField().getPassword())));
	...
	private class RegisterSignUpButtonListener implements ActionListener {	
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(LogRegView.getRegisterLoginField().getText().length() < 3) {
				JOptionPane.showMessageDialog(getLogRegView().getFrame(), "Login must be at least 3 digits", "Login too small", JOptionPane.WARNING_MESSAGE);
			} else if (LogRegView.getRegisterPasswordField().getPassword().length < 3){
				JOptionPane.showMessageDialog(getLogRegView().getFrame(), "Password must be at least 3 digits", "Password too small", JOptionPane.WARNING_MESSAGE);
			} else {
				setDataMessage(new SignUpMessage(LogRegView.getRegisterLoginField().getText(), 
						String.valueOf(LogRegView.getRegisterPasswordField().getPassword())));
				getThreadManager().sendMessage(getDataMessage());
			}
		}
	}
