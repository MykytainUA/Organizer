...
public class Server {
	...
	public void executeRequest() {
		try {
			...
			} else if (this.getMessage() instanceof AddNewTaskMessage) {
				System.out.println("SERVER:AddNewTaskMessage received");
				this.getLogManager().writeToLogs("SERVER:AddNewTaskMessage received");
				AddNewTaskMessage message = (AddNewTaskMessage) this.getMessage();
				
				int statusOfInsert = this.getDatabaseManager().tryToCreateNewTask(message);
				
				if(statusOfInsert == 0) {
					message.success();
				} else if (statusOfInsert == 1) {
					message.dateAlreadyOccupied();
				} else {
			...
			} else if (this.getMessage() instanceof GetAllTasksForThisUserMessage) {
				System.out.println("SERVER:GetAllTasksForThisUserMessage received");
				this.getLogManager().writeToLogs("SERVER:GetAllTasksForThisUserMessage received");
				GetAllTasksForThisUserMessage message = (GetAllTasksForThisUserMessage) this.getMessage();
				this.getDatabaseManager().tryToGetAllTasksForUser(message);
				message.success();
				this.setMessage(message);
				this.getLogManager().writeToLogs("SERVER:Message received:");
				this.getLogManager().writeToLogs(message.toString());
				
			} else if (this.getMessage() instanceof DeleteSelectedTasksMessage) {
				System.out.println("SERVER:DeleteSelectedTasksMessage received");
				DeleteSelectedTasksMessage message = (DeleteSelectedTasksMessage) this.getMessage();
				int rowsAffected = this.getDatabaseManager().tryToDeleteSelectedTasks(message);
				if(rowsAffected != 0) {
					message.success();
				} else {
					message.noTasksWereDeleted();
				}
				this.setMessage(message);
				this.getLogManager().writeToLogs("SERVER:Message received:");
				this.getLogManager().writeToLogs(message.toString());
				
			} else if (this.getMessage() instanceof ChangeTasksStatesToOppositeMessage) {
				System.out.println("SERVER:ChangeTasksStatesToOppositeMessage received");
				ChangeTasksStatesToOppositeMessage message = (ChangeTasksStatesToOppositeMessage) this.getMessage();
				int rowsAffected = this.getDatabaseManager().tryToChangeTasksStatesToOpposites(message);
				if(rowsAffected != 0) {
					message.success();
				} else {
					message.noTasksWereChanged();
				}
				
				this.setMessage(message);
				this.getLogManager().writeToLogs("SERVER:Message received:");
				this.getLogManager().writeToLogs(message.toString());
			} else if (this.getMessage() instanceof ManageMultipleTasksMessage) {
				System.out.println("SERVER:ChangeTasksStatesToOppositeMessage received");
				ManageMultipleTasksMessage message = (ManageMultipleTasksMessage) this.getMessage();
				System.out.println(message);
				
				if(message.getAction().equals("ADD")) {
					this.getDatabaseManager().tryToAddMultipleTasks(message);
				} else if(message.getAction().equals("DELETE")) {
					this.getDatabaseManager().tryToDeleteMultipleTasks(message);
				} else {
					message.error();
					this.getLogManager().writeToLogs("SERVER:ERROR:Unknown action in message received!!!");
					this.getLogManager().writeToLogs(message.toString());
				}
				message.success();
				this.setMessage(message);
